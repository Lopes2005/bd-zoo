...
WITH
  ...
  plano_rentabilidade AS (
    -- Rentabilidade = Receita da Zona * (Votos do Recinto / Total de Votos da Zona)
    SELECT
      r.id_recinto,
      (
        rz.receita_total_zona * (r.votos * 1.0 / NULLIF(vz.total_votos_zona, 0))
      )::REAL AS rentabilidade_calculada
    FROM
      recinto r
...
UPDATE recinto r
SET
  rentabilidade = p.rentabilidade_calculada
FROM
  plano_rentabilidade p
WHERE
  r.id_recinto = p.id_recinto;